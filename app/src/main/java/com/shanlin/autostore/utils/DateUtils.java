...
public class DateUtils {
	...
	public static String howLongAgo(long time) {
		...
		if (timeAgo<60) {
			return AutoStoreApplication.getApp().getString(R.string.me_just);
		}
		if (timeAgo < 60 * 60) {
			return AutoStoreApplication.getApp().getString(R.string.me_minutes_ago, timeAgo / 60);
		}
		if (timeAgo < 24*60*60) {
			return AutoStoreApplication.getApp().getString(R.string.me_hour_ago,timeAgo / (60 * 60));
		}
		if (timeAgo < 30*24*60*60) {
			return AutoStoreApplication.getApp().getString(R.string.me_day_ago,timeAgo / (60 * 60 *24));
		}
		if (timeAgo<12*30*24*60*60) {
			return AutoStoreApplication.getApp().getString(R.string.me_month_ago,timeAgo / (60 * 60 *24 *30));
		}

		return AutoStoreApplication.getApp().getString(R.string.me_year_ago,timeAgo / (60 * 60 * 365));
	}
